...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        super.onCreate(b); setContentView(com.juken5.autoafr.R.layout.activity_main);
        ...
        initAxes(); initMap(); renderTable();
        findViewById(R.id.autoTuneStart).setOnClickListener(v->{tuning=true; status.setText("AUTO TUNE aktif — tunggu AFR stabil.");});
        ...
        findViewById(R.id.reset).setOnClickListener(v->{initMap();renderTable();});
    ...
    int cellRpm(double r){int c=(int)Math.round(r/250.0);return Math.max(0,Math.min(RPM_CELLS-1,c));}
    int cellTps(double t){int c=(int)Math.round(t/5.0);return Math.max(0,Math.min(TPS_CELLS-1,c));}
    ...
    void applySample(){
        ...
        double r=num(rpm,3000), t=num(tpsIn,20);
        int x=cellRpm(r), y=cellTps(t);
        double err=a-ta;
        if(Math.abs(err)<db){status.setText("AFR dalam deadband — koreksi 0%.");return;}
        // Positive correction adds fuel when measured AFR is leaner than target.
    ...
    void chooseBluetooth(){
        ...
        String[] names=new String[arr.length]; for(int i=0;i<arr.length;i++)names[i]=arr[i].getName()+" — "+arr[i].getAddress();
    ...
    void connect(BluetoothDevice d){
        new Thread(()->{
            try{
                ...
                socket.connect(); reader=new BufferedReader(new InputStreamReader(socket.getInputStream(),StandardCharsets.US_ASCII));
                ...
                rx=new Thread(()->readLoop());rx.start();
    ...
    void parseTelemetry(String s){
        // Supported telemetry line: AFR=13.8,RPM=3500,TPS=22.5
        try{
            ...
            if(a!=null&&r!=null&&t!=null){final double fa=a,fr=r,ft=t;runOnUiThread(()->{afr.setText(df.format(fa));rpm.setText(String.valueOf((int)fr));tpsIn.setText(df.format(ft));live.setText("LIVE AFR: "+df.format(fa)+" / "+df.format(num(target,13.8)));if(tuning)applySample();});}
    ...
    void exportCsv(){
        try{
            ...
            FileOutputStream o=new FileOutputStream(f);StringBuilder s=new StringBuilder("TPS/RPM");for(int x=0;x<RPM_CELLS;x++)s.append(",").append(rpms[x]);s.append("\n");
